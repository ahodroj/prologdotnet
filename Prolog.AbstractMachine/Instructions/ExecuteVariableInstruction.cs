...
{
    public class ExecuteVariableInstruction : AbstractInstruction
    {
        private string _vn;
        ...
        public override void Execute(AbstractMachineState state)
        {
            ...
            if (X0.IsList)
            {
                throw new Exception("executevar: cannot call a list.");
            }
            else if (X0.IsReference)
            {
                throw new Exception("executevar: cannot call an unbound variable.");
            }
            else if (X0.IsConstant)
            {
                ...
                {
                    throw new Exception("executevar: cannot call an int.");
                }
